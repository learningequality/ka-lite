...
def impl(context):
    assert reverse(
        "student_view") in context.browser.current_url, "Assertion failed. {url} not in {current_url}".format(
...
def impl(context, learner, progress_verbs, exercise):
    ...
    if not progress_verbs == "not started":
        ...
        if progress_verbs == "completed":
            ...
            log.attempts = 25
        for i in range(0, log.attempts):
            attempt_log, created = AttemptLog.objects.get_or_create(exercise_id=exercise, user=user, seed=i,
                                                                    timestamp=datetime.datetime.now())
        log.latest_activity_timestamp = datetime.datetime.now()
        log.save()
...
def impl(context, exercise, learner, progress_text, progress_colour):
    ...
    assert cell.text == progress_text, "Progress text for {learner}, on {exercise} incorrect".format(learner=learner,
                                                                                                     exercise=exercise)
...
def impl(context):
    ...
    assert len(context.browser.find_elements_by_css_selector(".headrow.data")) == 10, len(
        context.browser.find_elements_by_css_selector(".headrow.data"))
