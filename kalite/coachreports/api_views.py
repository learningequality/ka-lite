...
def return_log_type_details(log_type, topic_ids=None):
    ...
    if topic_ids:
        objects = [obj for topic_id in topic_ids for obj in
                   get_topic_contents(topic_id=topic_id, kinds=[log_type.title()])]
...
def learner_logs(request):
    ...
    start_date = datetime.datetime.strptime(start_date, '%Y/%m/%d') if start_date else end_date - datetime.timedelta(
        time_window)
    ...
    for log_type in log_types:
        ...
        if not topic_ids:
            ...
            objects = dict([(item.get("id"), item) for item in
                            get_topic_nodes(ids=[obj[id_field] for obj in topic_objects]) or []]).values()
...
def aggregate_learner_logs(request):
    ...
    for log_type in log_types:
        ...
        if log_type == "video":
            ...
            output_dict["content_time_spent"] += log_objects.aggregate(Sum("total_seconds_watched"))[
            ...
            if log_objects.aggregate(Avg("streak_progress"))["streak_progress__avg"] is not None:
                output_dict["exercise_mastery"] = round(
                    log_objects.aggregate(Avg("streak_progress"))["streak_progress__avg"])
        output_logs.extend(log_objects)
    ...
    if len(all_object_ids) > 0:
        output_dict["available_topics"] = map(lambda x: {"id": x.get("id"), "title": x.get("title")},
    ...
    learner_event_objects = dict([(item["id"], item) for item in get_topic_nodes(
        ids=[getattr(log, "exercise_id", getattr(log, "video_id", getattr(log, "content_id", ""))) for log in
             output_logs[:event_limit]], language=request.language) or []])
    ...
    output_dict["learner_events"] = [{
                                         ...
                                         "content": learner_event_objects.get(getattr(log, "exercise_id",
                                                                                      getattr(log, "video_id",
