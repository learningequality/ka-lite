...
from kalite.topic_tools.content_models import get_content_item, get_topic_nodes_with_children, get_topic_contents, \
    get_content_items
...
def get_resume_recommendations(user, request):
    ...
    if final:
        return [get_content_item(language=request.language, channel=getattr(final, "channel", "khan"),
...
def get_next_recommendations(user, request):
    ...
    if current_subtopic:
        topic_tree_based_data = generate_recommendation_data()[current_subtopic]['related_subtopics'][
                                :settings.TOPIC_RECOMMENDATION_DEPTH]
...
def get_group_recommendations(user):
    ...
    if recent_exercises:
        ...
        exercise_counts = ExerciseLog.objects.filter(user__in=user_list).values("exercise_id").annotate(
            count=Count("exercise_id"))
...
def get_explore_recommendations(user, request):
    ...
    recent_subtopics = list(
        set([exercise_parents_table[ex]['subtopic_id'] for ex in recent_exercises if ex in exercise_parents_table]))
    ...
    for subtopic_id in random_subtopics:
        ...
        recommended_topic = next(
            topic for topic in related_subtopics if topic not in added and topic not in recent_subtopics)
    ...
    for topic in topicId_list:
        if topic:
            exercises = get_topic_contents(topic_id=topic, kinds=["Exercise"])[
                        :5]  # can change this line to allow for more to be returned
            for e in exercises:
...
def get_most_recent_incomplete_item(user):
    ...
    exercise_list = list(
        ExerciseLog.objects.filter(user=user, complete=False).order_by("-latest_activity_timestamp")[:1])
...
def get_most_recent_exercises(user):
    ...
    exercises_by_user = ExerciseLog.objects.filter(user=user).order_by("-latest_activity_timestamp").values_list(
        "exercise_id", flat=True)
            ...
            if data[left]['related_subtopics'][1] != ' ':
                ...
                if (at_four_left):
                    ...
                    if data[curr]['related_subtopics'][1].split(' ')[1] == '4':
                        at_four_left = True
                        new_dist = 4
                    elif data[left]['related_subtopics'][1].split(' ')[
                        1] == '4':  # if the next left neighbor is at dist 4
                        at_four_left = True
                        new_dist = 4
            ...
            if data[right]['related_subtopics'][2] != ' ':
                ...
                if (at_four_right):
                    ...
                    if data[curr]['related_subtopics'][2].split(' ')[1] == '4':
                        new_dist = 4
                    elif data[right]['related_subtopics'][2].split(' ')[
                        1] == '4':  # if the next right neighbor is at dist 4
                        new_dist = 4
