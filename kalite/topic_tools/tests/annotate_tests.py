...
class AnnotateTestCase(TestCase):
  ...
  def setUp(self, db=None):
    ...
    thestuff = [u'{"sha": "c2074685910760669c855d628b7a44ef8d828be9", "live": true, "id": "xde8147b8edb82294"}',
 ...
 u'{"sha": "446eef76c02fc29049012312c704c12b4b54ba8d", "live": true, "id": "x9ef4ca7e914c87ec"}']
  ...
  def test_update_content_availability_true(self):
    expected = {
      "thepath" :  {'all_assessment_items': [],
              'description': 'test',
              'title': 'testing '}      
    }
    with Using(self.db, [Item]):
      actual = update_content_availability([unparse_model_data(model_to_dict(self.item))])
  ...
  '''def test_khan_exercises_availiability_true(self):
    expected = {

    }
    actual = update_content_availability()
    self.assertEqual(expected, actual)


  def test_khan_exercises_availiability_false(self):
    expected = {
