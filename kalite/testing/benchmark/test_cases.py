...
class OneHundredRandomLogUpdates(base.UserCommon):
    ...
    def _execute(self):
        for x in range(50):
            ...
            this_video.save()
            this_exercise = ExerciseLog.objects.get(
                id=self.exercise_list[int(self.random.random() * self.exercise_count)].id)
            this_exercise.save()
...
class SeleniumStudent(base.SeleniumCommon):
    ...
    def _setup(self, starttime="00:00", duration=30, **kwargs):
        ...
        def wait_until_starttime(starttime):
            ...
            if self.verbosity >= 1:
                print("(" + str(
                    self.behavior_profile - 24601) + ") waiting until it's time to start (%.1fs)." % time_to_sleep)
    ...
    def _setup_activities(self):
        ...
        self.activity["wv2"] = {
            ...
            "args": {
                "path": "/math/arithmetic/addition-subtraction/two_dig_add_sub/v/adding-whole-numbers-and-applications-1/"},
            "nextstep": [(1.00, "wv2_1")]
        }
        self.activity["wv2_1"] = {
    ...
    def _execute(self):
        ...
        while True:
            ...
            if "duration" in self.activity[current_activity]:
                if self.verbosity >= 2:
                    print "(" + str(self.behavior_profile - 24601) + ")" + "sleeping for ", \
                    self.activity[current_activity]["duration"]
            ...
            for threshold, next_activity in self.activity[current_activity]["nextstep"]:
                if threshold >= next_activity_random:
                    if self.verbosity >= 2:
                        print "(" + str(self.behavior_profile - 24601) + ")" + str(
                            next_activity_random), "next_activity =", next_activity
