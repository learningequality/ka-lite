...
def count_existing_videos(topic, db_name):
    ...
    # Only look for videos if there are more branches
        ...
        if "children" in topic["children"][0]:
            for child in topic["children"]:
                child['nvideos'] = count_existing_videos(topic=child, db_name=db_name)
                nvideos += child['nvideos']
        ...
        # All my children are leaves, so we'll query here (a bit more efficient than 1 query per leaf)
            ...
            if len(videos) > 0:
                ...
                nvideos = len(found_videos)
    
#    import logging
#    logging.warn('nvideos: %d'%nvideos)    
    return nvideos
