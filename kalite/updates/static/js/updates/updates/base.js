...
function updateDisplay(process_name, progress_log) {
    ...
    if (progress_log.completed) {
        ...
        select_update_elements(process_name, "#stage-summary").text(sprintf("Overall progress: %(percent_complete)5.1f%% complete (%(cur_stage)d of %(num_stages)d )", {
            cur_stage: parseInt(progress_log.cur_stage_num),
            num_stages: parseInt(progress_log.total_stages),
            percent_complete: parseInt(100*progress_log.process_percent)
        }));
