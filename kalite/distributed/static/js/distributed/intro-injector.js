...
//------------------------------------------------------------------
function attachElements(jsonResponse) {
  var url = Object.keys(jsonResponse);
  //var parsedResponse = {}; don't need to return a parsed response when attaching elements------

  //Parse the array containing selector objects for a given URL
  var newselector = [];
  newselector = _.map(jsonResponse[url], function(element) {
    var selectorKey = Object.keys(element);   //the element that we will be appending attributes to
    var attributes = [];
    attributes = element[selectorKey];  
