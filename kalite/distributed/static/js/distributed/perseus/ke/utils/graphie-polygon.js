...
$.extend(KhanUtil, {
    Polygon: function (numSides) {
        var angles = [], points = [], gExteriorAngles = [];
        ...
        (function () {
            var graph = KhanUtil.currentGraph, curr, length, min = 1, max = 5, incr = 1;
            for (var i = 0; i < numSides - 2; i++) {
                var evenlyDivided = 180 / numSides, jitter = KhanUtil.randRange(-10, 10) / 40;
            ...
            while (points.length !== angles.length + 2) {
                ...
                points = [
                    ...
                    graph.polar(KhanUtil.randRange(min, max), curr)
                ];
        ...
        function drawDiagonalTriangle(start) {
            var graph = KhanUtil.currentGraph, length = points.length;
            graph.style({
        ...
        this.drawDiagonals = function (start) {
            var graph = KhanUtil.currentGraph, p1 = points[start % points.length];
        ...
        this.drawRadialDiagonals = function () {
            var graph = KhanUtil.currentGraph, cx = 0, cy = 0;
        ...
        this.drawExteriorAngles = function () {
            var graph = KhanUtil.currentGraph, prevTheta = 0, prevPoint;
            ...
            $.each(points, function (index, point) {
                if (index !== 0) {
                    var distance = getDistance(prevPoint, point), dx = point[0] - prevPoint[0], dy = point[1] - prevPoint[1], theta = Math.acos(dx / distance) * 180 / Math.PI, coord;
                    if (dy < 0) {
                        theta = 360 - theta;
                    }
        ...
        function getColor(i) {
            switch (i % 4) {
            case 0:
                return KhanUtil.BLUE;
            case 1:
                return KhanUtil.ORANGE;
            case 2:
                return KhanUtil.GREEN;
            case 3:
                return KhanUtil.PINK;
            }
        }
        this.animateExteriorAngles = function (start) {
            var graph = KhanUtil.currentGraph, origin = graph.scalePoint(points[start]);
            points.push([
                0,
                0
            ]);
            ...
            for (var i = 1; i < gExteriorAngles.length; i++) {
                var gAngle = gExteriorAngles[i], point = points[i], coord = graph.scalePoint(point), clone = gAngle.attr('stroke', getColor(i)).clone();
                clone.animate({
                    translation: [
                        origin[0] - coord[0],
                        origin[1] - coord[1]
    ...
    Circle: function (radius, center) {
        ...
        this.drawPoint = function (theta, label) {
            var graph = KhanUtil.currentGraph, point = graph.polar(radius, theta);
            if (label) {
        ...
        this.drawChord = function (theta1, theta2) {
            var graph = KhanUtil.currentGraph, point1 = graph.polar(radius, theta1), point2 = graph.polar(radius, theta2);
            return graph.line(point1, point2);
        };
        this.drawLabel = function (label, theta) {
            var graph = KhanUtil.currentGraph, coord = graph.polar(radius + 0.4, theta);
        ...
        this.drawMovablePoint = function (theta, min, max) {
            ...
            graph.graph.inscribedPoint = graph.addMovablePoint({
                coordX: point[0],
                coordY: point[1]
            });
        ...
        this.drawCentralArc = function (start, end, arcRadius) {
            var graph = KhanUtil.currentGraph, arc;
        ...
        this.drawInscribedArc = function (inscribed, start, end, arcRadius) {
            var graph = KhanUtil.currentGraph, vertex = graph.polar(radius, inscribed), point1 = graph.polar(radius, start), point2 = graph.polar(radius, end), theta1 = getThetaFromXY(point1[0] - vertex[0], point1[1] - vertex[1]), theta2 = getThetaFromXY(point2[0] - vertex[0], point2[1] - vertex[1]), arc;
            arcRadius = arcRadius || 0.5;
            graph.style({ fill: '' }, function () {
        ...
        this.drawInscribedAngle = function (inscribed, start, end, arcRadius, label) {
            var chords = [
                    this.drawChord(inscribed, start),
                    this.drawChord(inscribed, end)
                ], vertex = this.drawPoint(inscribed), arc = this.drawInscribedArc(inscribed, start, end, arcRadius), label = this.drawLabel(label, inscribed);
            return {
                chords: chords,
                vertex: vertex,
                arc: arc,
