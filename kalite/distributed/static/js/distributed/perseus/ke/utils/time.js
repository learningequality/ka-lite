...
$.extend(KhanUtil, {
        ...
        analogClock.drawHands = function () {
            ...
            self.graph.style({
                ...
                var hourHand = self.graph.path([
                    [
                        self.center[0] - 0.15 * self.radius,
                        self.center[1] + 0.035 * self.radius
                    ],
                    [
                        self.center[0] + 0.45 * self.radius,
                        self.center[1] + 0.02 * self.radius
                    ],
                    [
                        self.center[0] + 0.45 * self.radius,
                        self.center[1] - 0.02 * self.radius
                ...
                hourHand.rotate(-KhanUtil.timeToDegrees((self.hour + self.minute / 60) * 5), self.graph.scalePoint(self.center)[0], self.graph.scalePoint(self.center)[1]);
                self.hands.push(hourHand);
                var minuteHand = self.graph.path([
                    [
                        self.center[0] - 0.15 * self.radius,
                        self.center[0] + 0.035 * self.radius
                    ],
                    [
                        self.center[0] + 0.85 * self.radius,
                        self.center[0] + 0.017 * self.radius
                    ],
                    [
                        self.center[0] + 0.85 * self.radius,
                        self.center[0] - 0.017 * self.radius
                ...
                minuteHand.rotate(-KhanUtil.timeToDegrees(self.minute), self.graph.scalePoint(self.center)[0], self.graph.scalePoint(self.center)[1]);
            ...
            self.graph.style({
                ...
                _(12).times(function (n) {
                    var tick = self.graph.line([
                        self.center[0] + self.radius - 4 / self.graph.scale[0],
                        self.center[1]
                    ], [
                        self.center[0] + self.radius - 8 / self.graph.scale[0],
                        self.center[1]
                    ]);
    ...
    addInteractiveAnalogClock: function (options) {
        ...
        var outerPointRadius = interactiveAnalogClock.radius * 1.1;
        ...
        interactiveAnalogClock.getHandPositions = function () {
            return [
                interactiveAnalogClock.minutePoint.coord,
                interactiveAnalogClock.hourPoint.coord
            ];
        };
        interactiveAnalogClock.validate = function (guess, correctMinute, correctHour) {
            ...
            if (minuteAngle !== correctMinuteAngle || hourAngle !== correctHourAngle) {
                ...
                } else if (minuteAngle === correctMinuteAngle && hourAngle !== correctHourAngle && hourAngle === KhanUtil.roundToNearest(hourSnapDegrees, KhanUtil.timeToDegrees(5 * correctHour))) {
