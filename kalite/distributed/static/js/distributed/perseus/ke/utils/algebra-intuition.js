...
KhanUtil.scale = {
    ...
    equation: {
        ...
        init: function (options) {
            ...
            $.extend(this, options);
            this.graphie.raphael.image(KhanUtil.scale.image, this.graphie.scalePoint([
                -7.8,
                this.yPos + 1.2
            ])[0], this.graphie.scalePoint([
                -7.8,
                this.yPos + 1.2
            ])[1], 40, 40).attr({ opacity: 1 });
        },
        draw: function (x, n, a, b) {
            ...
            if (a + x === b) {
                this.eqLabel = this.graphie.label([
                    0,
                    this.yPos
                ], '\\Huge{=}');
            } else {
            ...
            if (n !== 1) {
                this.leftCoefLabel = this.graphie.label([
                    -6.8,
                    this.yPos
                ], '\\Huge{' + n + '(\\qquad\\quad)}');
            } else {
            ...
            if (a !== 0) {
                this.leftLabel = this.graphie.label([
                    -3.7,
                    this.yPos
                ], '\\Huge{{} + ' + a + '}');
            } else {
            ...
            this.rightLabel = this.graphie.label([
                3.5,
                this.yPos
            ], '\\Huge{' + b + '}');
        }
    ...
    addWeight: function (xOffset, yOffset, num, dish) {
        ...
        _(10).times(function (n) {
            ...
            weight.lbl = KhanUtil.scale.graphie.raphael.text(KhanUtil.scale.graphie.scalePoint([
                pos[0] + xOffset + 0.5,
                pos[1] + yOffset + 0.5
            ])[0], KhanUtil.scale.graphie.scalePoint([
                pos[0] + xOffset + 0.5,
                pos[1] + yOffset + 0.5
            ])[1], '1');
            weight.lbl.attr({
    ...
    balance: function (speed) {
        ...
        var newAngle = 0;
        this.equation.draw(this.unknown, this.leftCoef, this.leftConst, this.rightConst);
        ...
        $({ r: this.angle }).animate({ r: newAngle }, {
            ...
            step: function (now, fx) {
                var dist = 5.5 * Math.sin(now * Math.PI / 180) * KhanUtil.currentGraph.scale[1];
                var leftPos = KhanUtil.scale.leftDishItems[0].attr('translation').y;
                var rightPos = KhanUtil.scale.rightDishItems[0].attr('translation').y;
                KhanUtil.scale.angle = now;
                KhanUtil.scale.beamItems.rotate(now, [
                    0,
                    0
                ]);
                KhanUtil.scale.leftDishItems.translate([
                    0,
                    -dist - leftPos
                ]);
                KhanUtil.scale.rightDishItems.translate([
                    0,
                    dist - rightPos
                ]);
            }
        ...
        if (_.isFunction(this.onChange)) {
            this.onChange(this.leftCoef, this.leftConst, this.rightCoef, this.rightConst);
        }
    },
    ...
    init: function (options) {
        ...
        this.leftDishItems.push(this.graphie.raphael.image(this.image, this.graphie.scalePoint([
            -8.9,
            this.baseY + 2.4
        ])[0], this.graphie.scalePoint([
            -8.9,
