...
function Adder(a, b, decimalA, decimalB) {
    ...
    var pos = {
        max: Math.max(digitsA.length, digitsB.length, KhanUtil.digits(a + b).length),
        carry: 3,
        first: 2,
        second: 1,
        sum: 0,
    ...
    this.showHint = function () {
        ...
        if (prevCarry !== 0) {
            highlights.push(graph.label([
                x,
                pos.carry
            ], '\\blue{' + prevCarry + '}', 'below'));
        ...
        this.showSideLabel('\\Large{' + prevCarryStr + '\\blue{' + KhanUtil.localeToFixed(digitsA[index] * power, decimalPlaces) + '}' + addendStr + ' = ' + resultStr + '}');
        index++;
    };
    this.showFinalCarry = function () {
        ...
        this.showSideLabel('\\Large{' + '\\blue{' + carry + '}' + ' = ' + '\\green{' + carry + '}' + '}');
    };
...
function Subtractor(a, b, digitsA, digitsB, decimalPlaces) {
    ...
    this.borrow = function (idx) {
        ...
        highlights[idx].push(graph.label([
            pos.max - idx,
            pos.carry + 0.5 * depth
        ], '\\blue{' + workingDigitsA[idx] + '}', 'below'));
        highlights[idx].push(graph.path([
            [
                pos.max - 0.3 - idx,
                pos.first - 0.4
            ],
        ...
        highlights[idx + 1].push(graph.label([
            pos.max - 1 - idx,
            pos.carry + 0.5 * depth
        ], '\\orange{' + workingDigitsA[idx + 1] + '}', 'below'));
        highlights[idx + 1].push(graph.path([
            [
                pos.max - 1.3 - idx,
                pos.first - 0.4
            ],
        ...
        if (depth !== 0) {
            highlights[idx + 1].push(graph.path([
                [
                    pos.max - 1.3 - idx,
                    pos.carry - 1 + 0.5 * depth
                ],
                [
                    pos.max - 0.7 - idx,
                    pos.carry - 0.7 + 0.5 * depth
                ]
            ]));
        }
        return borrowedIdx;
    ...
    this.showHint = function () {
        ...
        if (value < subtrahend) {
            ...
            highlights[index].push(graph.label([
                pos.max - index,
                pos.first
        ...
        this.showSideLabel('\\Large{' + '\\blue{' + workingDigitsA[index] * power + '}' + subStr + ' = ' + '\\green{' + diff + '}' + zeros + '}');
        index++;
    };
    this.getNumHints = function () {
        return numHints;
    ...
    this.showDecimals = function (deciA, deciB) {
        for (var i = 0; i < 3; i++) {
            graph.style({ fill: '#000' }, function () {
                graph.label([
                    pos.max - Math.max(deciA, deciB) + 0.5,
                    i - 0.1
                ], '\\LARGE{' + decimalPointSymbol + '}', 'center', true);
            });
        }
...
function drawRow(num, y, color, startCount) {
    ...
    for (var x = 0; x < num; x++) {
        set.push(graph.label([
            x,
            y
        ], '\\small{\\color{' + color + '}{' + (startCount + x) + '}}'));
        set.push(graph.circle([
            x,
            y
        ], 0.35));
    }
...
function Multiplier(a, b, digitsA, digitsB, deciA, deciB) {
    ...
    this.show = function () {
        ...
        graph.label([
            -Math.max(digitsA.length, digitsB.length),
            1
        ], '\\LARGE{\\times\\vphantom{0}}');
    };
    this.removeHighlights = function () {
        while (highlights.length) {
            highlights.pop().remove();
    ...
    this.showHint = function () {
        ...
        if (carry) {
            highlights = highlights.concat(graph.label([
                -indexA,
                3
            ], '\\purple{' + carry + '}', 'below'));
        }
        ...
        graph.label([
            2,
            2 - indexB * digitsA.length - indexA
        ], hint, 'right');
        var result = [ones];
        if (indexA === 0) {
...
function Divider(divisor, dividend, deciDivisor, deciDividend, decimalRemainder) {
    ...
    this.show = function () {
        ...
        graph.style({ fill: '#000' }, function () {
            if (deciDivisor !== 0) {
                decimals = decimals.concat(graph.label([
                    -1 - deciDivisor,
                    -0.1
                ], '\\LARGE{' + decimalPointSymbol + '}', 'center', true));
            }
            if (deciDividend !== 0) {
                decimals = decimals.concat(graph.label([
                    digitsDividend.length - deciDividend - 0.5,
                    -0.1
                ], '\\LARGE{' + decimalPointSymbol + '}', 'center', true));
            }
        });
    ...
    this.shiftDecimals = function () {
        ...
        temporaryLabel = graph.label([
            dx,
            1
        ], $.ngettext('\\text{Shift the decimal 1 to the right.}', '\\text{Shift the decimal %(num)s to the right.}', deciDivisor), 'right');
        this.addDecimals([
            [
                -1,
                -0.1
            ...
            [
                digitsDividend.length + deciDiff - 0.5,
                -0.1
            ]
        ]);
        if (deciDiff > 0) {
    ...
    this.showDivisionStep = function (division) {
        var question = $._('\\text{How many times does }%(divisor)s' + '\\text{ go into }\\color{#6495ED}{%(value)s}\\text{?}', {
            divisor: divisor,
            value: currentValue
        });
        ...
        if (!fOnlyZeros) {
            graph.style({ arrows: '->' }, function () {
                highlights.push(graph.path([
                    [
                        index,
                        -0.5
                    ],
    ...
    this.showDivisionStepResult = function (result, remainder) {
        ...
        if (result !== 0) {
            ...
            graph.label([
                dx,
                dy + 1
            ], '\\blue{' + currentValue + '}' + '\\div' + divisor + '=' + '\\green{' + result + '}' + '\\text{' + $._(' with a remainder of ') + ' }' + '\\pink{' + remainder + '}', 'right');
        }
        index++;
    };
    this.addDecimalRemainder = function () {
        ...
        graph.label([
            dx,
            1
        ], $._('\\text{Write in a decimal and a zero.}'), 'right');
    };
