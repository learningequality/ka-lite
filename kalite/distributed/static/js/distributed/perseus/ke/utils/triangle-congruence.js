...
$.extend(KhanUtil, {
    initCongruence: function (options) {
        ...
        if (options.type === 'SSS') {
            ...
            triangle.points[0].constraints.fixedDistance = {
                dist: options.sides[0],
                point: triangle.points[1]
            };
            triangle.setRotationPoint(1);
            triangle.setRotationPoint(2);
            triangle.points[3].constraints.fixedDistance = {
                dist: options.sides[2],
            ...
            triangle.setRotationPoint(1);
            triangle.points[2].constraints.fixedDistance = {
                dist: options.sides[1],
                point: triangle.points[1]
            };
            triangle.points[3].constraints.fixedAngle = {
                angle: options.angles[2] * (triangle.reflected ? 1 : -1),
                vertex: triangle.points[2],
            ...
            triangle.snapCorrect = function () {
                ...
                if (Math.abs(KhanUtil.getDistance(triangle.points[2].coord, triangle.points[3].coord) - options.triangle.sideLengths[2]) > 1) {
            ...
            triangle.points[1].constraints.fixedDistance = {
                dist: options.sides[1],
                point: triangle.points[2]
            };
            triangle.setRotationPoint(2);
            ...
            triangle = KhanUtil.addInteractiveTriangle(options);
            triangle.setRotationPoint(0);
            triangle.points[1].constraints.fixedAngle = {
                angle: options.angles[2] * (triangle.reflected ? -1 : 1),
                vertex: triangle.points[2],
                ref: triangle.points[3]
            };
            triangle.setRotationPoint(2);
            triangle.points[3].constraints.fixedAngle = {
                angle: options.angles[2] * (triangle.reflected ? 1 : -1),
            ...
            triangle = KhanUtil.addInteractiveTriangle(options);
            triangle.points[0].constraints.fixedAngle = {
                angle: options.angles[1] * (triangle.reflected ? -1 : 1),
                vertex: triangle.points[1],
                ref: triangle.points[2]
            };
            ...
            triangle.points[3].onMove = function (coordX, coordY) {
                ...
                triangle.points[0].setCoord([
                    coordX,
                    coordY
                ]);
                var scaleFactor = KhanUtil.getDistance([
                    coordX,
                    coordY
                ], triangle.rotationPoint.coord) / triangle.radii[3];
    ...
    addInteractiveTriangle: function (options) {
        ...
        triangle.update = function () {
            if (!KhanUtil.dragging) {
                ...
                for (var point = 0; point < 4; ++point) {
                    ...
                    if (triangle.points[point].isRotationPoint) {
                        triangle.points[point].constraints.fixedDistance = {
                            dist: triangle.radii[point],
                            point: triangle.rotationPoint
                        };
                    }
                }
            ...
            for (var angle = 0; angle < 2; ++angle) {
                $(triangle.arcs[angle]).each(function () {
                    this.remove();
                });
            ...
            $(triangle.points).each(function () {
                this.toFront();
            });
        };
        triangle.setRotationPoint = function (point) {
            triangle.points[point].isRotationPoint = true;
            triangle.points[point].normalStyle = {
                fill: KhanUtil.BLUE,
                stroke: KhanUtil.BLUE,
                scale: 1
            };
            triangle.points[point].highlightStyle = {
                fill: KhanUtil.BLUE,
                stroke: KhanUtil.BLUE,
                scale: 1.5
            };
            ...
            triangle.points[point].onMove = function (coordX, coordY) {
                var dAngle = KhanUtil.findAngle([
                    coordX,
                    coordY
                ], triangle.points[point].coord, triangle.rotationPoint.coord) * Math.PI / 180;
                for (var i = 0; i < 4; ++i) {
                    if (i !== point) {
        ...
        $('.question').prepend('<button id=\'reflect\' type=\'button\'>' + $._('Reflect shape') + '</button>');
        $('button#reflect').bind('click', function (event) {
            this.blur();
            if (!triangle.animating) {
                triangle.animating = true;
                var startPoints = $.map(triangle.points, function (pt) {
                    return [pt.coord.slice()];
                });
                var xMin = Math.min.apply(Math, $.map(startPoints, function (x) {
                    return x[0];
                }));
                var xMax = Math.max.apply(Math, $.map(startPoints, function (x) {
                    return x[0];
                }));
                var xMid = (xMin + xMax) / 2;
                var endPoints = $.map(triangle.points, function (pt) {
                    return [[
                            xMid - pt.coord[0] + xMid,
                            pt.coord[1]
                        ]];
                });
                ...
                triangle.reflected = !triangle.reflected;
                $(triangle.arcs[0]).each(function () {
                    this.remove();
                });
                $(triangle.arcs[1]).each(function () {
                    this.remove();
                });
                $(triangle.arcs[2]).each(function () {
                    this.remove();
                ...
                $(xCoords).animate({
                    ...
                    step: function (now, fx) {
                        $(triangle.points).each(function (n) {
                            this.setCoord([
                                xCoords[n],
                                endPoints[n][1]
                            ]);
        ...
        triangle.points.push(graphie.addMovablePoint({
            coord: coord,
            bounded: false
        }));
        ...
        triangle.lines.push(graphie.addMovableLineSegment({
            pointA: triangle.points[0],
            pointZ: triangle.points[1],
        ...
        triangle.lines.push(graphie.addMovableLineSegment({
            pointA: triangle.points[1],
            pointZ: triangle.points[2],
        ...
        triangle.lines.push(graphie.addMovableLineSegment({
            pointA: triangle.points[2],
            pointZ: triangle.points[3],
            ticks: options.ticks[2],
            highlightStyle: {
                'stroke': KhanUtil.BLUE,
                'stroke-width': 4
            }
    ...
    addTriangleDecorations: function (triangle, type) {
        ...
        var graphie = KhanUtil.currentGraph;
        graphie.addMovableLineSegment({
            coordA: triangle.points[0],
            coordZ: triangle.points[1],
            fixed: true,
            ticks: ticks[0],
            normalStyle: {
                stroke: '#b1c9f5',
        ...
        graphie.addMovableLineSegment({
            coordA: triangle.points[1],
            coordZ: triangle.points[2],
            fixed: true,
            ticks: ticks[1],
            normalStyle: {
                stroke: '#b1c9f5',
        ...
        graphie.addMovableLineSegment({
            coordA: triangle.points[2],
            coordZ: triangle.points[0],
            fixed: true,
            ticks: ticks[2],
            normalStyle: {
                stroke: '#b1c9f5',
