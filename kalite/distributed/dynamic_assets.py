...
class DynamicSettings(DynamicSettingsBase):
    fixed_block_exercises = fields.IntegerField(default=getattr(settings, "FIXED_BLOCK_EXERCISES", 0), minimum=0,
                                                maximum=10)
    quiz_repeats = fields.IntegerField(default=getattr(settings, "QUIZ_REPEATS", 0), minimum=1, maximum=10)
    turn_off_motivational_features = fields.BooleanField(
        default=getattr(settings, "TURN_OFF_MOTIVATIONAL_FEATURES", False))
    ...
    streak_correct_needed = fields.IntegerField(default=getattr(settings, "STREAK_CORRECT_NEEDED", 8), minimum=5,
                                                maximum=10)
