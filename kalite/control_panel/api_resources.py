...
class ExerciseLogResource(ParentFacilityUserResource):
    ...
    def alter_list_data_to_serialize(self, request, to_be_serialized):
        ...
        for bundle in to_be_serialized["objects"]:
            ...
            #Anything done before Nov 15, 2014 is not in RCT so unit 99 to differentiate
            if not bundle.data["timestamp_first"] or bundle.data["timestamp_first"] <= datetime(2014, 11, 15, 0, 0, 0):
                bundle.data["unit"] = 99

            # Anything done after Nov 15, 2014 is in the RCT which starts from Unit 101
            elif StoreTransactionLog.objects.filter(user=user, context_type="unit_points_reset", purchased_at__gte=datetime(2014, 11, 15, 0, 0, 0)).count() == 0:
                bundle.data["unit"] = 101

            elif bundle.data["timestamp_first"]:
                try:
                    transaction = StoreTransactionLog.objects.filter(user=user, context_type="unit_points_reset", purchased_at__gte=bundle.data["timestamp_first"]).order_by("purchased_at")[0]
                    bundle.data["unit"] = int(transaction.context_id)
                except:
                    transaction = StoreTransactionLog.objects.filter(user=user, context_type="unit_points_reset", purchased_at__lt=bundle.data["timestamp_first"]).order_by("-purchased_at")[0]
                    bundle.data["unit"] = int(transaction.context_id) + 1
