...
def save_cache_file(cache_type, cache_object=None, node_cache=None, data_path=None):
    if cache_object is not None:
        ...
        logging.debug(
            "Saving %(cache_type)s to %(dest_filepath)s" % {"cache_type": cache_type, "dest_filepath": dest_filepath})
        ...
        for node in kind_nodes.values():
            for att in channel_data["temp_ok_atts"]:
                if att in node:
                    ...
                    if att == "live":
                        assert node[att] == True, "All non-live nodes (%s) better be deleted by this point!" % node[
                            "id"]
                    del node[att]
