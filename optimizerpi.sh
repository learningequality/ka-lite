...
echo 'CONFIG_PACKAGE = "RPi"' >> $current_dir/kalite/local_settings.py
...
sudo sh -c "$current_dir/kalite/manage.py nginxconfig > /etc/nginx/sites-enabled/kalite" 
...
sudo cat > /etc/nginx/nginx.conf <<'NGINX'
user www-data;
